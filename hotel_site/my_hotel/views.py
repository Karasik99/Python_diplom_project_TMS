...
def support(request, id1, id2):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            mail = send_mail(form.cleaned_data['subject']+' (Отправитель: '+get_mail[0]+')', form.cleaned_data['content'], 'companyvira@yandex.by', ['nikita.carasevitch@yandex.by'], fail_silently=False)
            if mail:
